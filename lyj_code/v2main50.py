...
if __name__ == "__main__":
    ...
    while(line<400):
        ...
        pretrained = False
        # 模型定义-ResNet
        net = resnetV2.resnet50(pretrained).to(device)
        if pretrained == True:
            weight_path = "/data/lyj/LNM202203/weight/50-i17_E05_1l0.4000_1w5.000_2l0.0500_2w50.000_t0.568_v0.657_e0.573_e0.555.pth"
        ...
        for epoch in range(pre_epoch, EPOCH):
            ...
            if auc_train>=0.6 and (auc_val>=0.69) and (auc_exter2>0.52) and auc_exter1>0.52:
                #if (auc_train - auc_test<0.1):
                    #if epoch>=2:
                        print('Saving model......')
                        torch.save(net.state_dict(), '%s/dp4_50-i%01d_E%02d_t%.03f_v%.03f_e%.03f_e%.03f_'
                                                     '1l%.04f_1w%.03f_2l%.04f_2w%.03f.pth'
                                   % (outfolder, line, epoch + 1,auc_train, auc_val, auc_exter1, auc_exter2,
                                      LR1*1000, weight_decay1*100, LR2*1000, weight_decay2*100
                                      ))
