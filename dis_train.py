...
from models import UNet, mtihead_Unet, resnet34, resnet50, resnet101
...
def train_net(net,
    ...
    val_set = LNM_Dataset('validation_8_1.csv', mode='val')
    ...
    for epoch in range(1, epochs+1):
        ...
        dice_score, val_acc = evaluate(model=net,
                            data_loader=val_loader,
                            device=device,
                            global_step=global_step,
                            experiment=experiment,
                            epoch=epoch)
        val_loss = dice_score + val_acc
        scheduler.step(val_loss)

        if rank == 0:
            logging.info('Validation Dice score: {}'.format(dice_score))
            logging.info('Validation Classification Acc: {}'.format(val_acc))
            # print("[epoch {}] accuracy: {}".format(epoch, round(acc, 3)))
            tags = ["loss", "Dice score", "Acc","learning_rate"]
...
if __name__ == '__main__':
    ...
    net = mtihead_Unet(n_channels=1, n_classes=args.classes, bilinear=args.bilinear)
    if rank ==0:
